...
export default {
  ...
  methods: {
    setUpPanel() {
      ...
      for (const list of [this.multiVariableList, this.linkVariableList]) {
        ...
        for (const attr of list) {
          ...
          const variableSvgWidth =
            variableSvg.node().getBoundingClientRect().width -
            this.yAxisPadding -
            this.varPadding;

          // Get the data and generate the bins
          const currentData = this.graphStructure[`${type}s`].map(
            (d) => d[attr],
          );
          const bins = currentData.reduce(
          ...
          const variableSvgEnter = variableSvg
            ...
            .attr('fill', (d) =>
              this.isQuantitative(attr, type)
                ? '#82B1FF'
                : this.nodeColorScale(d),
            );
          ...
          const brush = brushX()
            .extent([
              [this.yAxisPadding, 0],
              [variableSvgWidth, this.svgHeight],
            ])
            .on('start brush', () => {
              const extent = event.selection;

              // Set the brush highlighting on the legend svg
              variableSvgEnter.attr('stroke', (d) =>
                xScale(d) >= extent[0] - xScale.bandwidth() &&
                xScale(d) <= extent[1]
                  ? '#000000'
                  : '',
              );
              ...
              if (attr === this.colorVariables[0]) {
                const start = binLabels.indexOf(
                  this.ordinalInvert(extent[0], xScale, binLabels),
                );
                const end = binLabels.indexOf(
                  this.ordinalInvert(extent[1], xScale, binLabels),
                );
          ...
          variableSvg.call(brush).call(brush.move, xScale.range());
        }
      }
    },
    ...
    isQuantitative(varName, type) {
      const uniqueValues = [
        ...new Set(
          this.graphStructure[`${type}s`].map((node) =>
  ...
  <div id="legend">
    <v-card>
      ...
      <svg class="sticky" height="33%" width="100%">
        ...
        <!-- Node elements -->
        <g id="nodeMapping">
          <text font-size="16pt" y="-102" dominant-baseline="hanging">
            Node Mapping
          </text>
          ...
          >
            ...
            <text class="barLabel" font-size="10pt" dominant-baseline="hanging">
              Bars
            </text>
            <path
