...
describe('Node-Link Utils', () => {
  describe('Arc Drawing', () => {
    ...
    beforeEach(() => {
      wrapper = mount(NodeLink, {
        propsData: {
          graphStructure: {
            nodes: [],
            links: [{ source: { x: 100, y: 150 }, target: { x: 400, y: 450 } }],
          },
          app: App,
    ...
    it('A straight arc returns the expected path', () => {
      ...
      // Assert
      expect(arc).toEqual(
        `M ${link.source.x + wrapper.vm.nodeMarkerLength / 2} ${
          link.source.y + wrapper.vm.nodeMarkerHeight / 2
        } L ${link.target.x + wrapper.vm.nodeMarkerLength / 2} ${
          link.target.y + wrapper.vm.nodeMarkerHeight / 2
        }`,
      );
    });
    ...
    it('A curved arc returns the expected path', () => {
      ...
      expect(arc).toEqual(
        `M ${link.source.x + wrapper.vm.nodeMarkerLength / 2}, ${
          link.source.y + wrapper.vm.nodeMarkerHeight / 2
        } A ${dr}, ${dr} ${xRotation}, ${largeArc}, ${sweep} ${
          link.target.x + wrapper.vm.nodeMarkerLength / 2
        },${link.target.y + wrapper.vm.nodeMarkerHeight / 2}`,
      );
    });
  });
